...
const GitHubCalendar = ({ username = 'Samuel-AKINGENEYE' }) => {
  ...
  // Fetch real contributions from GitHub Events API
  const fetchContributions = async (year) => {
    setLoading(true);
    setError(null);
    
    try {
      // Fetch user's events (this gives us real contribution data)
      ...
      // Count contributions by date for the selected year
      const dailyCount = {};
      let total = 0;
      ...
      events.forEach(event => {
        ...
        if (yearStr === year.toString()) {
          // Count push events, create events, and pull request events
      ...
      // Build weekly grid (53 weeks max)
      const weeks = [];
      ...
      while (currentDate.getDay() !== 0 && currentDate <= endDate) {
        currentDate.setDate(currentDate.getDate() + 1);
      }
      // Move back to first day of year for proper alignment
  ...
  const generateMockData = (year) => {
    ...
    while (currentDate <= endDate) {
      // Simulate realistic contribution pattern (more on weekdays)
      const dayOfWeek = currentDate.getDay();
      let count = 0;
      if (dayOfWeek !== 0 && dayOfWeek !== 6) {
        // Weekdays have higher chance of contributions
        ...
        <div className="animate-pulse space-y-4">
          <div className="flex justify-between">
            ...
            <div className="flex gap-2">
              {[1,2,3,4,5].map(i => <div key={i} className="h-8 w-14 bg-slate-200 dark:bg-slate-700 rounded"></div>)}
            </div>
          </div>
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700">
      <div className="flex flex-wrap justify-between items-center gap-3 mb-6">
        <div>
          <span className="text-2xl font-bold text-slate-900 dark:text-white">{totalCount.toLocaleString()}</span>
