            ...
            <div key={p._id} className="flex items-center gap-4 p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="font-medium text-slate-900 dark:text-white text-sm truncate">{p.title}</p>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div><label className={LABEL}>Category</label><select value={form.category} onChange={e => set('category', e.target.value)} className={INPUT}><option>AI/ML</option><option>Web Dev</option><option>Cybersecurity</option><option>Other</option></select></div>
      </div>
...
function CertificatesTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const res = await certificatesApi.getAll(); setCertificates(res.data.data ?? []); }
...
function SkillsTab() {
  ...
  return (
    <div>
      ...
      {modal && <Modal title={modal === 'add' ? 'Add Skill' : `Edit: ${modal.name}`} onClose={() => setModal(null)}><SkillForm initial={modal !== 'add' ? modal : null} onSave={handleSave} onCancel={() => setModal(null)} /></Modal>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      {loading ? <SkeletonRows /> : (
        <div className="space-y-3">
          {items.map(item => (
            <div key={item._id} className="flex items-center justify-between p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl">
              <div className="flex-1">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setModal(item)} className="p-2 rounded-lg text-slate-400 hover:text-blue-500"><Edit2 size={14} /></button>
                <button onClick={async () => { if (confirm('Delete?')) { await educationApi.remove(item._id); toast.success('Deleted'); load(); } }} className="p-2 rounded-lg text-slate-400 hover:text-red-500"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      {modal && (
        <Modal title={modal === 'add' ? 'Add Education' : `Edit: ${modal.institution}`} onClose={() => setModal(null)}>
          <EducationForm initial={modal !== 'add' ? modal : null} onSave={async (data) => { if (modal === 'add') await educationApi.create(data); else await educationApi.update(modal._id, data); toast.success('Saved!'); setModal(null); load(); }} onCancel={() => setModal(null)} />
        </Modal>
      )}
    </div>
  );
...
function ExperienceForm({ initial, onSave, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div><label className={LABEL}>Description</label><textarea rows={3} value={form.description} onChange={e => set('description', e.target.value)} className={INPUT} /></div>
      <div><label className={LABEL}>Achievements (comma-separated)</label><textarea rows={2} value={form.achievements} onChange={e => set('achievements', e.target.value)} className={INPUT} placeholder="Built X feature, Improved Y by Z%" /></div>
      <div className="flex gap-3 pt-2">
        <button type="submit" disabled={saving} className={BTN_PRIMARY + ' flex-1'}>{saving ? <Spinner /> : <Save size={14} />}{saving ? 'Saving…' : 'Save'}</button>
        <button type="button" onClick={onCancel} className={BTN_GHOST}>Cancel</button>
      </div>
    ...
    <div>
        ...
        <div className="space-y-3">
          {items.map(item => (
            <div key={item._id} className="flex items-center justify-between p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setModal(item)} className="p-2 rounded-lg text-slate-400 hover:text-blue-500"><Edit2 size={14} /></button>
                <button onClick={async () => { if (confirm('Delete?')) { await experienceApi.remove(item._id); toast.success('Deleted'); load(); } }} className="p-2 rounded-lg text-slate-400 hover:text-red-500"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      {modal && (
        <Modal title={modal === 'add' ? 'Add Experience' : `Edit: ${modal.position} at ${modal.company}`} onClose={() => setModal(null)}>
          <ExperienceForm initial={modal !== 'add' ? modal : null} onSave={async (data) => { if (modal === 'add') await experienceApi.create(data); else await experienceApi.update(modal._id, data); toast.success('Saved!'); setModal(null); load(); }} onCancel={() => setModal(null)} />
        </Modal>
      )}
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-xl">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div><label className={LABEL}>Years of Experience</label><input type="number" value={form.yearsOfExperience ?? 0} onChange={e => set('yearsOfExperience', Number(e.target.value))} className={INPUT} /></div>
      </div>
      <div>
      ...
      <header className="sticky top-0 z-40 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
        <div className="max-w-5xl mx-auto px-6 h-14 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <Link to="/" target="_blank" className="text-xs text-slate-500 hover:text-blue-500 hidden sm:block">
              View site →
            </Link>
            <DarkModeToggle />
            <button
              onClick={handleLogout}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm text-slate-500 hover:bg-red-50 hover:text-red-600 transition-colors"
            >
              <LogOut size={14} />
