...
import {
  ...
  ArrowDown, Send, Award, TrendingUp, Users, Layers,
} from 'lucide-react';
...
const STATS = [
  { icon: Layers, label: 'Projects Built', value: 8, suffix: '+' },
  { icon: Users, label: 'Users Reached', value: 500, suffix: '+' },
  { icon: TrendingUp, label: 'Years Experience', value: 2, suffix: '+' },
  { icon: Award, label: 'Certifications', value: 5, suffix: '' },
];

const TYPING_STRINGS = ['Full Stack Developer', 'React Specialist', 'API Builder', 'Problem Solver'];
...
function useTypingEffect(strings, typingSpeed = 90, pauseDuration = 1800) {
  ...
  }, [displayed, typing, idx, strings, typingSpeed, pauseDuration]);
...
function StatCard({ icon: Icon, label, value, suffix }) {
  ...
  return (
    <div ref={ref} className="text-center p-6 card rounded-2xl glow-hover">
      <div className="w-12 h-12 rounded-xl bg-blue-500/10 flex items-center justify-center mx-auto mb-3">
        <Icon size={22} className="text-blue-500" />
      </div>
      <p className="text-3xl font-extrabold text-slate-900 dark:text-white stat-num">
        {count}{suffix}
      </p>
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <button type="submit" disabled={sending}
        className="w-full py-3 rounded-xl bg-blue-500 hover:bg-blue-600 text-white font-medium text-sm
                   transition-all duration-200 hover:scale-[1.02] hover:shadow-lg hover:shadow-blue-500/25
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen">
      ...
      >
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] rounded-full
                          bg-blue-500/8 blur-3xl animate-float" />
          <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] rounded-full
                          bg-purple-500/6 blur-3xl animate-float"
               style={{ animationDelay: '1.5s' }} />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
                          w-[600px] h-[600px] rounded-full bg-cyan-500/4 blur-3xl" />
        </div>
        ...
        <div className="container-max w-full py-20 relative z-10">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div>
              ...
              <h1 className="text-5xl lg:text-6xl font-extrabold text-slate-900 dark:text-white leading-tight mb-4 animate-slide-up">
                Hi, I'm{' '}
              ...
              {/* Typing effect */}
              <div className="h-8 mb-3 animate-slide-up-d1">
                <p className="text-xl font-semibold text-slate-600 dark:text-slate-300">
                  {typedText}
                  <span className="inline-block w-0.5 h-5 bg-blue-500 ml-0.5 animate-blink" />
                </p>
              </div>

              <p className="text-slate-500 dark:text-slate-400 leading-relaxed mb-8 max-w-lg animate-slide-up-d2">
              ...
              <div className="flex flex-wrap gap-4 mb-10 animate-slide-up-d3">
                <a
                  href="#projects"
                ...
                <a
                  ...
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-xl
                             ...
                             font-medium transition-all duration-200 hover:scale-[1.03]"
                >
            ...
            <div className="hidden lg:flex justify-center">
              <div className="relative animate-float">
                ...
                <div className="relative w-80 h-80 rounded-3xl overflow-hidden border border-blue-500/30
                                bg-gradient-to-br from-slate-800 via-slate-900 to-slate-800
                                flex items-center justify-center shadow-2xl">
                  <span className="text-8xl font-black text-blue-500/20 select-none tracking-tight">SA</span>
          ...
          <div className="flex justify-center mt-16">
            <a href="#stats" className="text-slate-300 dark:text-slate-600 hover:text-blue-500 transition-colors animate-bounce"
      ...
      {/* ═══════════════════ STATS ═══════════════════ */}
      <section id="stats" className="py-16 px-6 bg-white dark:bg-slate-800/30 border-y border-slate-100 dark:border-slate-700/50">
      ...
      {/* ═══════════════════ SKILLS ═══════════════════ */}
      <Section id="skills" className="bg-white dark:bg-slate-800/30">
        <SectionHead
          title="Skills & Tech Stack"
          sub="Tools and technologies I use to build products"
        />
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {Object.entries(SKILLS).map(([category, { icon: Icon, color, bg, items }], i) => (
            <div key={category} className={`card p-6 glow-hover reveal reveal-d${i + 1}`}>
      ...
      <Section id="testimonials">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {TESTIMONIALS.map((t, i) => (
            <div key={i} className={`card p-6 glow-hover flex flex-col reveal reveal-d${i + 1}`}>
              <div className="mb-3">
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCerts.map((c, i) => (
              <div key={c._id} className={`reveal reveal-d${Math.min(i + 1, 4)}`}>
      ...
      <Section id="about">
        ...
        <div className="grid lg:grid-cols-2 gap-14 items-start">
          ...
          <div className="card p-6 reveal reveal-d2">
            <h3 className="font-semibold text-slate-900 dark:text-white mb-5">Quick Facts</h3>
            <dl className="space-y-4">
              {[
                { icon: Mail, color: 'bg-blue-500/10 text-blue-500', label: 'Email', value: profile?.email ?? 'freshtalent491@gmail.com' },
