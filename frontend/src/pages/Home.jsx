    ...
    <div className="min-h-screen">
        ...
        <div className="relative z-10 max-w-6xl mx-auto w-full py-20">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="animate-slide-up">
              ...
              <div className="text-xl md:text-2xl lg:text-3xl font-semibold text-slate-300 mb-4 min-h-[2.5rem]">
                <TypingText texts={HERO_TEXTS} />
              </div>
              
              <p className="text-slate-400 leading-relaxed mb-6 max-w-lg text-sm md:text-base">
                {profile?.bio ||
              ...
              <div className="flex flex-wrap gap-8">
                {[
                  ...
                  <div key={label}>
                    <p className="text-3xl font-extrabold text-white stat-num">
            ...
            <div className="hidden lg:flex justify-center">
              <div className="relative animate-float">
                ...
                <div className="relative w-80 h-80 rounded-[2rem] overflow-hidden border border-blue-500/20 shadow-2xl bg-slate-900">
                  {profile?.avatar ? (
                    <img src={profile.avatar} alt={profile.name || 'Profile'} className="w-full h-full object-cover animate-float" loading="eager" />
                  ) : (
        ...
        <div className="container-max">
          ...
          {allTechs.length > 1 && (
            <div className="flex flex-wrap gap-2 justify-center mb-10 reveal">
              {allTechs.map(tech => (
                <button key={tech} onClick={() => setTechFilter(tech)} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all border ${techFilter === tech ? 'bg-blue-500 text-white border-blue-500 shadow-lg shadow-blue-500/20' : 'border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-blue-400 hover:text-blue-500'}`}>
                  {tech}
                </button>
              ))}
            </div>
          ...
          {loading ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredProjects.map((p, i) => <div key={p._id} className={`reveal reveal-d${Math.min(i + 1, 4)}`}><ProjectCard project={p} /></div>)}
            </div>
          )}
        </div>
      </section>
      ...
      <section id="skills" className="section">
        <div className="container-max">
          ...
          {loading ? (
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">{[1,2,3,4].map(i => <SkeletonCard key={i} className="h-48" />)}</div>
                ...
                return (
                  <div key={cat} className={`reveal reveal-d${Math.min(i + 1, 4)} card-glass glow-hover p-6 border ${cfg.border}`}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {catSkills.map(skill => <span key={skill._id} className={`px-3 py-1.5 rounded-full text-xs font-medium ${cfg.bg} ${cfg.color} border ${cfg.border} hover:scale-105 transition-transform`}>{skill.name}</span>)}
                    </div>
                  </div>
                );
              })}
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {Object.entries({ Frontend: { icon: Globe, color: 'text-blue-500', bg: 'bg-blue-500/10', items: ['React', 'Next.js', 'TypeScript', 'Tailwind CSS'] }, Backend: { icon: Code2, color: 'text-green-500', bg: 'bg-green-500/10', items: ['Node.js', 'Express', 'Python'] }, Database: { icon: Database, color: 'text-purple-500', bg: 'bg-purple-500/10', items: ['PostgreSQL', 'MongoDB', 'Redis'] }, Tools: { icon: Wrench, color: 'text-orange-500', bg: 'bg-orange-500/10', items: ['Git', 'Docker', 'AWS'] } }).map(([cat, { icon: Icon, color, bg, items }], i) => (
                <div key={cat} className={`reveal reveal-d${i + 1} card-glass glow-hover p-6 border`}>
                  ...
                  <div className="flex flex-wrap gap-2">{items.map(skill => <span key={skill} className={`px-3 py-1.5 rounded-full text-xs font-medium ${bg} ${color} border`}>{skill}</span>)}</div>
                </div>
              ))}
            </div>
          )}
      ...
      <section id="experience" className="section bg-white dark:bg-slate-800/20">
        <div className="container-max">
          ...
          {loading ? <div className="space-y-6 max-w-3xl mx-auto">{[1,2].map(i => <SkeletonCard key={i} className="h-36" />)}</div> : experience.length > 0 ? <div className="max-w-3xl mx-auto"><Timeline items={experience} type="experience" /></div> : <p className="text-center text-slate-500 text-sm py-8">No experience entries yet.</p>}
        </div>
      </section>
      ...
      <section id="education" className="section">
        <div className="container-max">
          ...
          {loading ? <div className="space-y-6 max-w-3xl mx-auto">{[1,2].map(i => <SkeletonCard key={i} className="h-28" />)}</div> : education.length > 0 ? <div className="max-w-3xl mx-auto"><Timeline items={education} type="education" /></div> : <p className="text-center text-slate-500 text-sm py-8">No education entries yet.</p>}
        </div>
      </section>
      ...
      <section id="certificates" className="section bg-white dark:bg-slate-800/20">
        <div className="container-max">
          ...
          {loading ? <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">{[1,2,3].map(i => <SkeletonCard key={i} className="h-48" />)}</div> : certificates.length > 0 ? <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">{certificates.map((cert, i) => <div key={cert._id} className={`reveal reveal-d${Math.min(i + 1, 4)}`}><CertificateCard certificate={cert} /></div>)}</div> : <p className="text-center text-slate-500 text-sm py-8">No certificates added yet.</p>}
        </div>
      </section>
      ...
      <section id="testimonials" className="section">
        <div className="container-max">
          ...
          <div className="grid md:grid-cols-3 gap-8">{TESTIMONIALS.map((t, i) => <div key={i} className={`reveal reveal-d${i + 1} card-glass glow-hover p-6`}><StarRating rating={t.rating} /><p className="text-sm italic mt-3 mb-4">"{t.feedback}"</p><div className="pt-3 border-t"><p className="font-semibold text-sm">{t.author}</p><p className="text-xs text-slate-500 mt-0.5">{t.role}</p></div></div>)}</div>
        </div>
      </section>
        ...
        <div className="container-max">
          ...
          <div className="grid lg:grid-cols-2 gap-12 max-w-4xl mx-auto">
            <form onSubmit={handleContact} className="reveal space-y-4">
              ...
              <div><label className="block text-sm font-medium mb-1.5">Name</label><input required value={contactForm.name} onChange={e => setContactForm(f => ({ ...f, name: e.target.value }))} placeholder="Your full name" className="input" /></div>
              <div><label className="block text-sm font-medium mb-1.5">Email</label><input required type="email" value={contactForm.email} onChange={e => setContactForm(f => ({ ...f, email: e.target.value }))} placeholder="you@example.com" className="input" /></div>
              <div><label className="block text-sm font-medium mb-1.5">Message</label><textarea required rows={5} value={contactForm.message} onChange={e => setContactForm(f => ({ ...f, message: e.target.value }))} placeholder="Tell me about your project..." className="input resize-none" /></div>
              <button type="submit" disabled={sending} className="w-full inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-blue-500 hover:bg-blue-600 disabled:opacity-60 text-white font-medium transition-all">{sending ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />}{sending ? 'Sending…' : 'Send Message'}</button>
            </form>
